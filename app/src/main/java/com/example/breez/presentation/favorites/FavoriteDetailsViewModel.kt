...
class FavoriteDetailsViewModel @Inject constructor(
    ...
    private
    val _showError = MutableSharedFlow<String>()
    val showError: SharedFlow<String> =
        _showError.asSharedFlow()
    ...
    fun loadWeatherData(lat: Double, lon: Double, favoriteId: Long? = null) {
        viewModelScope.launch {
            ...
            when {
                currentWeatherResult is ApiResult.Success && forecastResult is ApiResult.Success -> {
                ...
                currentWeatherResult is ApiResult.Error -> {
                    _uiState.value =
                        FavoriteDetailsUiState.Error(message = currentWeatherResult.message)
                    _showError.emit(currentWeatherResult.message)
                }

                forecastResult is ApiResult.Error -> {
                    _uiState.value =
                        FavoriteDetailsUiState.Error(message = forecastResult.message)
