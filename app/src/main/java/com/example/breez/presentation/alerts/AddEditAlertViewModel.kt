...
class AddEditAlertViewModel @Inject constructor(
    ...
    private val _uiState = MutableStateFlow<AddEditAlertUiState>(
        if (alertId != null) {
            AddEditAlertUiState.Loading
        } else {
            AddEditAlertUiState.Form()
        }
    )
    ...
    private fun loadAlert(id: Long) {
        viewModelScope.launch {
            try {
                ...
                _uiState.value = AddEditAlertUiState.Error(
                    "Failed to load alert: ${e.message}"
                )
            }
        }
    }
    ...
    private fun prefetchLocation() {
        viewModelScope.launch {
            try {
                ...
                val geo = repository.getLocationNameFromCoordinates(
                    loc.first,
                    loc.second
                )

                val city = (geo as? ApiResult.Success)
                    ?.data
                    ?.firstOrNull()
                ...
                // Silent fail because location can be optional at this stage.
            }
        }
    }

    fun updateAlertType(type: AlertType) = withForm {
        copy(alertType = type)
    }

    fun updateStartTime(startTime: Date) = withForm {
        val newEndTime = if (isOneTimeAlert) startTime else endTime
    ...
    fun toggleAlertMode(isOneTime: Boolean) = withForm {
        val newEndTime = if (isOneTime) startTime else endTime
        ...
        viewModelScope.launch {
            try {
                ...
                if (loc != null) {
                    ...
                    withForm {
                        copy(
                            locationError = "Unable to get current location. Make sure GPS is enabled."
                        )
                    }
                }
            } catch (e: Exception) {
    ...
    fun saveAlert() {
        ...
        if (!state.isValid()) {
            viewModelScope.launch {
                _showToast.emit("Please check alert time and location")
            }
            return
        }
        ...
        viewModelScope.launch {
            ...
            try {
                val latestState = _uiState.value as? AddEditAlertUiState.Form ?: return@launch
                val entity = latestState.toAlertEntity()

                val savedId = if (latestState.isEditing) {
    ...
    private fun withForm(
        block: AddEditAlertUiState.Form.() -> AddEditAlertUiState.Form
    ) {
        val state = _uiState.value

        if (state is AddEditAlertUiState.Form) {
